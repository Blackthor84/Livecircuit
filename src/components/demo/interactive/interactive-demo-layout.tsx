...
const NAV = [
  { href: "/demo/interactive", label: "Home", icon: Sparkles, exact: true },
  { href: "/demo/interactive/artist", label: "Artist", icon: Mic2 },
  { href: "/demo/interactive/fan", label: "Fan", icon: Ticket },
  { href: "/demo/interactive/agency", label: "Agency", icon: Building2 },
  { href: "/demo/interactive/sponsor", label: "Sponsor", icon: Users },
  { href: "/demo/interactive/arena", label: "Arena", icon: Sparkles },
  { href: "/demo/interactive/map", label: "Map", icon: Map },
];

export function InteractiveDemoLayout({ children }: { children: React.ReactNode }) {
  const pathname = usePathname();
  const isFinale = pathname.includes("/finale");
  const isHub = pathname === "/demo/interactive";
      ...
      <header className="sticky top-0 z-50 border-b border-white/5 bg-background/60 backdrop-blur-xl">
        <div className="mx-auto flex max-w-7xl items-center justify-between gap-4 px-4 py-3 sm:px-6">
          <div className="flex items-center gap-3">
            {!isHub ? (
              <Link href="/demo/interactive" className="rounded-lg p-2 text-muted-foreground transition hover:bg-white/5 hover:text-foreground">
                <ArrowLeft className="size-4" />
              </Link>
            ) : null}
            <Link href="/demo/interactive" className="flex items-center gap-2">
          ...
          <nav className="hidden items-center gap-1 md:flex">
            {NAV.map((item) => {
              const active = item.exact ? pathname === item.href : pathname.startsWith(item.href) && item.href !== "/demo/interactive";
              return (
                <Link
                  key={item.href}
                  href={item.href}
